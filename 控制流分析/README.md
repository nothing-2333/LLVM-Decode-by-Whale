...
- CFG.h:
  ...
  bool isCriticalEdge(const Instruction *TI,        // 基本块的终止指令(如分支、跳转指令)
                      ...
                      bool AllowIdenticalEdges = false); // 是否允许 “相同前驱” 的特殊情况
...
- CFG.cpp:
  ...
  bool llvm::isPotentiallyReachable(
    ...
    if (A->getParent() == B->getParent()) {
      ...
      // 入口块不可能是 EntryBlock(虚构出来的起始块), 若 A 在 B 之后则不可达
      if (BB->isEntryBlock())
        return false;
      ...
      // 检查是否可从后继块到达 B 所在的块, 也就是看看能不能绕回来
...
构建 cfg 在 `clang/lib/CodeGen/CodeGenFunction.h` 中完成, 上边的代码提供的功能是为了进行控制流分析. 分析其中一些关键函数.

### FindFunctionBackedges
识别所有循环回边. 有回边就是循环结构, 实现上很简单, 图的深度优先遍历.

其中 `succ_begin` 与 `succ_end` 描述了一个后继块构成的数组, 类似 vector 迭代器的遍历.

三个核心数据结构:
  - Visited: 记录已访问过的基本块, 避免重复处理;
  - VisitStack: 存储 <当前块, 待处理后继迭代器>, 实现 DFS 的栈式遍历;
  - InStack: 记录当前遍历路径中的块, 若某后继块已在 InStack 中, 说明该边是「从路径内块指向路径内前驱块」的回边, 直接加入结果.

### isCriticalEdge
临界边判断函数. 函数实现比较简单, 翻看前面的注释, 就是遍历, 判断前驱.

#### 临界边定义
从 “有多个后继的基本块” 指向 “有多个前驱的基本块” 的控制流边. 这类边会导致优化(如代码提升、寄存器分配)难以进行, 通常需要被 “拆分”(插入空基本块). 

### isPotentiallyReachable 与 isPotentiallyReachableFromMany
可达性分析函数. 判断「从起点到终点是否存在控制流路径」, 采用保守策略：仅当能 100% 证明 “不可达” 时返回 false, 否则(包括不确定的情况)返回 true, 避免误判导致优化错误. 提供三个重载, 覆盖 “指令 - 指令” “块 - 块” “多起点 - 单终点” 三种场景.

### containsIrreducibleCFG
检测 CFG 是否可规约. CFG 规约核心价值是通过简化控制流结构, 降低编译器优化和程序分析的复杂度, 确保循环识别、数据流分析、结构化代码生成等关键步骤的高效性和正确性. 

#### 可规约与不可规约 CFG 的定义(书中 143 页)
- 可规约控制流图: 所有循环都只有一个 “循环头”, 且所有进入循环的控制流边(入边)都必须指向这个循环头. 每一个循环只能通过它的首节点进入.
- 不可规约控制流图: 存在至少一个 “不可约循环”—— 即循环有多个入口点(且这些入口点不都经过同一个循环头), 或者说存在两条及以上从循环外到循环内的路径, 且这它们的 “最后公共前驱” 在循环外. 

#### 实现逻辑
- 1. 遍历传进来的每个节点, 用 Visited 记录已遍历节点; 
- 2. 对每个节点的后继, 若后继已在 Visited 中(说明该边是回边):
  - 检查回边是否属于 LI 记录的 “可约回边”(即存在包含起点块、且头为终点块的循环)
  - 若回边不可约, 返回 true(CFG 不可约)
- 3. 遍历结束无不可约回边：返回 false(CFG 可约). 
